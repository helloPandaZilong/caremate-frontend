...
function getNavConfig(path, userRole) {
  ...
  if (path.startsWith("/shop") || (isCommonPath && roleType === "REPAIR_SHOP")) {
    return {
      items: SHOP_NAV,
      role: "수리점 파트너",
  ...
  if (path.startsWith("/admin") || (isCommonPath && roleType === "ADMIN")) {
    return {
      items: ADMIN_NAV,
      role: "관리자",
...
function DesktopSidebar({collapsed,onToggle,dark,onLogout,paymentOrderId,userRole}) {
...
function MobileTopNav({ dark, toggleDark, onLogout, paymentOrderId, userRole }) {
  const loc = useLocation();
  const { items, role, roleColorLight, roleColorDark, profileHref, profileLabel } =
      getNavConfig(loc.pathname, userRole);
  const roleColor = dark ? roleColorDark : roleColorLight;
...
function DesktopTopBar({ collapsed, dark, toggleDark, user }) {
  const loc = useLocation();
  const { profileHref } = getNavConfig(loc.pathname, user?.role);
...
export default function AppShell() {
  ...
  useEffect(() => {
    if (normalizeRole(user?.role) !== "CUSTOMER") return;
    let cancelled = false;
