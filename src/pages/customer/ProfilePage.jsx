...
export default function CustomerProfilePage() {
  ...
  useEffect(() => {
    if (tab === "insurance" && policies.length === 0) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const { data } = await updateProfile({ name, phoneNumber: phone });
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                value={profile?.email ?? ""}
                disabled
                className="px-3.5 py-2.5 text-sm bg-secondary border border-border rounded-xl text-muted-foreground cursor-not-allowed"
              />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-muted-foreground">
      ...
      {tab === "security" && (
        <div className="flex flex-col gap-4">
          <Card className="p-6 flex flex-col gap-5">
            ...
            <Button variant="accent" size="sm" className="self-end">
              비밀번호 변경
            </Button>
          </Card>
          ...
          <div className="flex flex-col divide-y divide-border">
            {NOTIF_SETTINGS.map((n) => (
              ...
              >
                ...
                >
                  <div
                    className={`absolute inset-0 rounded-full transition-colors ${notifs[n.key] ? "bg-accent" : "bg-border"}`}
                  />
                  <span
                    className="absolute top-0.5 left-0.5 bg-white rounded-full shadow-sm transition-all"
                    style={{
                      width: 18,
                      height: 18,
